...
A small **ZoiteChat / HexChat** Python addon that uploads a file to a **0x0.st-compatible** host and pastes the returned URL into the chat tab where you started the upload. Humanity did invent at least one useful shortcut.

## Features

- Upload files to a 0x0-compatible host
...
Opens a file chooser, uploads the selected file to the configured 0x0-compatible host, and sends the returned URL into the current chat tab.
...
* The upload host must be compatible with the 0x0.st response format.
